...
public class TodoDatabaseProvider {
	...
	public long writeTodoToDb(TodoDbHelper mDbHelper, TodoEntry entry) {
		...
		long newRowId;
		newRowId = db.insert(TodoDbEntry.TABLE_NAME, null, values);
	...
	public Cursor readTodoFromDb(TodoDbHelper mDbHelper, int catId) {
		...
		String[] projection = { TodoDbEntry._ID,
				TodoDbEntry.COLUMN_NAME_CATEGORY, TodoDbEntry.COLUMN_NAME_TEXT,
				TodoDbEntry.COLUMN_NAME_UPDATED };

		// How you want the results sorted in the resulting Cursor
		String sortOrder = TodoDbEntry.COLUMN_NAME_UPDATED + " DESC";

		String[] values = { catId + "" };

		Cursor c = db.query(TodoDbEntry.TABLE_NAME, // The table to query
				projection, // The columns to return
				TodoDbEntry.COLUMN_NAME_CATEGORY + "=?",// The columns for the
														// WHERE clause
	...
	public long writeCategoryToDb(TodoDbHelper mDbHelper, CategoryEntry entry) {
		...
		// Insert the new row, returning the primary key value of the new row
		long newRowId;
		newRowId = db.insert(CategoryDbEntry.TABLE_NAME, null, values);
		Log.d(TAG,
				"Adding Category " + newRowId + " " + entry.getCategoryName());
	...
	public Cursor readCategoryFromDb(TodoDbHelper mDbHelper) {
		...
		String[] projection = { CategoryDbEntry._ID,
				CategoryDbEntry.COLUMN_NAME_NAME };

		Cursor c = db.query(CategoryDbEntry.TABLE_NAME, // The table to query
				projection, // The columns to return
	...
	public int getNumOfEntriesInCategory(TodoDbHelper mDbHelper, int catId) {
		...
		int numOfEntries = (int) DatabaseUtils.queryNumEntries(db,
	...
	public ArrayList<CategoryEntry> readCategoryEntries(
		...
		while (cursor.moveToNext()) {
			...
			cat = cursor.getString(cursor
					.getColumnIndex(CategoryDbEntry.COLUMN_NAME_NAME));
	...
	public ArrayList<TodoEntry> readTodoEntries(int todoCategoryId,
		...
		while (cursor.moveToNext()) {
			...
			cat = cursor.getString(cursor
					.getColumnIndex(TodoDbEntry.COLUMN_NAME_CATEGORY));
			text = cursor.getString(cursor
					.getColumnIndex(TodoDbEntry.COLUMN_NAME_TEXT));
			id = cursor.getString(cursor.getColumnIndex(TodoDbEntry._ID));
			Log.d(TAG, "Read entry " + text + " in cat " + cat + " "
					+ todoCategoryId);
			entList.add(new TodoEntry(id, todoCategoryId, cat, text)); // TODO
																		// think
																		// about
																		// if
																		// its
